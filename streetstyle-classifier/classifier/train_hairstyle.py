...
from dataset_hairstyle import HairStyleDataset
from model_hairstyle import HairStyleClassifier
...
class HairStyleClassifierTest(BaseTest):
    ...
    def __init__(self, use_gpu=True):
        ...
        self.num_attributs = len(HairStyleDataset.attributes)
        self.log['best_model_val_mean_class_acc'] = [0] * self.num_attributs
        self.log['best_model_val_acc'] = [0] * self.num_attributs
        self.log['best_model_val_loss'] = [float('inf')] * self.num_attributs
        
#         pretrained_log = torch.load(streetstyle_model_path)
#         self.pretrained_model = pretrained_log['best_model']


    def create_data_loaders(self, image_data_dir, manifest_data_path, batch_size):
        ...
        dataset = HairStyleDataset(image_data_dir, manifest_data_path, batch_size, 
    ...
    def create_model(self, model_type):
        self.model = HairStyleClassifier(model_type)
        if self.use_gpu:
...
#         self.optimizer = optim.Adam(self.model.parameters(), lr=1e-2, weight_decay=0)
    ...
    def train_model(self, num_iters, **kwargs):
        ...
        running_loss = np.zeros((self.num_attributs)) # loss for each attribute
        running_correct = np.zeros((self.num_attributs)) # number correct classifications for each attribute
        running_total = np.zeros((self.num_attributs)) # total number of classifications attempted
        iter_count = 0
...
#                     self.log['best_model_val_mean_class_acc'] = np.ndarray.tolist(val_mean_class_acc)
    ...
    def test_model(self):
        ...
        class_correct = []
        class_total = []
        for i in range(self.num_attributs):
            num_attr_classes = len(HairStyleDataset.attributes[i])
